...
public class ItenaryController {
    ...
    public ResponseEntity<List<Itenary>> getItinerariesByTrip(@RequestParam String tripId) {
        ...
        List<Itenary> itineraries = itenaryService.getItinerariesByTripId(tripUuid);
        return ResponseEntity.ok(itineraries);
    }

    @GetMapping("/{id}")
    public ResponseEntity<Itenary> getItineraryById(@PathVariable UUID id) {
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Itenary> updateItinerary(@PathVariable UUID id, @RequestBody Itenary itenary) {
        return itenaryService.getItineraryById(id)
                .map(existing -> {
                    itenary.setId(id);
                    return ResponseEntity.ok(itenaryService.saveItinerary(itenary));
